...
export default function EditBooking() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({
        ...
        childrenFree: booking.childrenFree ?? 0,
        childrenHalf: booking.childrenHalf ?? 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setSaving(false);
      alert(`Failed to update booking: ${err?.error || res.statusText}`);
    }
  };
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="font-[family-name:var(--font-display)] text-3xl font-semibold text-dark mb-6">Edit Booking</h1>
      <form onSubmit={handleSubmit} className="bg-warm-white rounded-lg shadow-sm p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={handleDelete} className="px-6 py-2.5 bg-red-600 text-white rounded text-sm font-medium hover:bg-red-700 transition-colors cursor-pointer border-none ml-auto">
            Delete
          </button>
        </div>
      </form>
